...
class RandomOrganismCreator:
    ...
    def __init__(self, random_org_parameters, composition_space, constraints):
        ...
        #ACH allow structure with binary compositions in ternary search
        ...
        # parse the parameters and set to defaults if necessary
            ...
            if 'allow_binary' not in random_org_parameters:
                ...
                self.allow_binary = random_org_parameters['allow_binary']
    ...
    def get_pd_species_list(self, composition_space, constraints, random):
        ...
        Description:
            ...
            8. Checks that the resulting number of atoms satisfies the maximum
                ...
                of the endpoint compositions. ACH: Also optionally checks that the
                resulting composition is not a binary.
        """
        ...
        # ACH check the composition - don't allow binary if specified
        if not self.allow_binary:
            if len(random_composition.elements) == 2:
                return None
...
class FileOrganismCreator:
    ...
    def create_organism(self, id_generator, composition_space, constraints,
        ...
        """
        #print('these are the files:',self.files)
        #changes made here, took out the -1 06/24/2024
    ...
    def update_status(self):
        ...
        print('Organisms left for {}: {} '.format(
            self.name, self.number - self.num_made))
        # added + 1 06/24/2024
        #print('num_made:',self.num_made)
        #print('length of self.files:',len(self.files) + 1)
...
class WGANsg:
    def __init__(self, input_file):
        ...
        self.name = 'generative organism creator'
        self.path_to_folder = ' '
        self.files = []
        self.number = len(self.files)
        self.num_made = 0  # number of attempts (usually number of files given)
    ...
    def rescale_images(self, images,divisor_list,factor_list):
    ...
    def extract_dims(self, images, elem_list):
        ...
        return dims
    def extract_coords(self, images, dims, num_atoms):
        count = 0
        coords = []
        for dim in dims:
            if dim == num_atoms:
