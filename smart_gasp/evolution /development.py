...
class Constraints(object):
    ...
    def __init__(self, constraints_parameters, composition_space):
        ...
        Args:
            ...
            ACH: added self.default_allow_binary default True
            ACH: added self.default_allow_magmom default False
            ACH: added self.default_magom default None
        '''
        ...
        self.default_allow_binary = True
        self.default_allow_magmom = False
        ...
        # parse the parameters and set to defaults if necessary
            ...
            #ACH whether to allow binary compositions in a ternary search
            #ACH not meant to be applied to organisms in the initial population
            if 'allow_binary' not in constraints_parameters:
                self.allow_binary = self.default_allow_binary
            elif constraints_parameters['allow_binary'] in (None,
                                                               'default'):
                self.allow_binary = self.default_allow_binary
            else:
                self.allow_binary = constraints_parameters[
                    'allow_binary']

            #ACH whether to add 'MAGMOM' tag in the INCAR file
            if 'allow_magmom' not in constraints_parameters:
                self.allow_magmom = self.default_allow_magmom
            elif constraints_parameters['allow_magmom'] in (None,
                                                               'default'):
                self.allow_magmom = self.default_allow_magmom
            else:
                self.allow_magmom = constraints_parameters['allow_magmom']
                if constraints_parameters['allow_magmom'] == True:
                    if 'magmom' not in constraints_parameters:
                        raise KeyError("magmom should be specified in constraints, if allow_magmom is True")
                    else:
                        self.magmom = constraints_parameters['magmom']
      ...
      def set_all_to_defaults(self, composition_space):
        ...
        Args:
            composition_space: the CompositionSpace of the search
            ACH: added self.allow_binary
            ACH: added self.allow_magmom
        '''
        ...
        self.allow_binary = self.default_allow_binary
        self.allow_magmom = self.default_allow_magmom
        self.magmom = self.default_magmom
        self.set_all_mids_to_defaults(composition_space)
...
class Developer(object):
    ...
    #ACH
    def is_binary_and_allowed(self, organism, constraints):
        """
        Returns a boolean indicating whether a binary organism 
        is allowd by the constraint allow_binary
        ...
        """
        #ACH
        if constraints.allow_binary == False and organism.made_by!='file organism creator':
           if organism.cell.ntypesp == 2:
                print('Kicking organism '+str(organism.id) +' because binary')
                return False
           else:
                return True
        elif constraints.allow_binary == True:
           return True
        else:
           return True
