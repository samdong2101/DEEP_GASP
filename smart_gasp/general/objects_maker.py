...
def make_organism_creators(parameters, composition_space, constraints):
    ...
    # make the specified creators
        ...
        if 'from_files' in parameters['InitialPopulation']:
            file_organism_creator = organism_creators.FileOrganismCreator(
                    parameters['InitialPopulation']['from_files']['path_to_folder'])
            initial_organism_creators.append(file_organism_creator)
            if 'path_to_folder' not in parameters['InitialPopulation'][
                    'from_files']:
...
def make_vasp_energy_calculator(parameters, composition_space, geometry):
    ...
    if parameters['EnergyCode']['vasp'] is None:
        ...
        #ACH allow_magmom?
        #currently no check is performed to see if the supplied magmom dict is
        #sufficient or not
        magmom=None
        if 'allow_magmom' in parameters['Constraints']:
            if parameters['Constraints']['allow_magmom'] == True:
                if 'magmom' not in parameters['Constraints']:
                    raise KeyError("magmom should be specified in constraints, if allow_magmom is True")
                else:
                    magmom = parameters['Constraints']['magmom']
